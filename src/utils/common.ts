...
export const createOffer = (row: string) => {
  ...
  return {
    ...
    typeOfHousing:
      TypeOfHousing[
        capitalizeFirstLetter(typeOfHousing) as
          | 'Apartment'
          | 'House'
          | 'Room'
          | 'Hotel'
    ...
    goods: goods
      ...
      .map(
        (goodsName) =>
          OfferGood[
            stringToPascalCase(goodsName) as
              | 'Breakfast'
              | 'AirConditioning'
    ...
    user: {
      ...
      avatarPath: avatar,
      password,
      userType: UserType[userType as 'Pro' | 'NotPro'],
      favoriteOffersId: [],
    },
...
export const setIsFavoriteFlag = (
  offers: DocumentType<OfferEntity>[] | (DocumentType<OfferEntity> | null)[],
    ...
    transformProperty(property, data, (target: UnknownObject) => {
      if (Array.isArray(target[property])) {
        ...
        target[property] = paths.map((path) => {
          const rootPath = DEFAULT_STATIC_IMAGES.includes(path as string)
            ? staticPath
            : uploadPath;
