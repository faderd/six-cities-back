...
export default class OfferService implements OfferServiceInterface {
  ...
  public async create(dto: CreateOfferDto): Promise<DocumentType<OfferEntity>> {
    const result = await this.offerModel.create({
      ...dto,
      commentsCount: 0,
      rating: RatingRange.DEFAULT_VALUE,
      isFavorite: false,
    });
  ...
  public async findIsPremiumByCity(
    city: string,
    count?: number,
  ...
  public async findIsFavoriteByUserId(
    userId: string,
  ): Promise<DocumentType<OfferEntity>[]> {
    const favoriteOffersId =
      (await this.userService.getFavoriteIdsByUserId(userId)) || [];
  ...
  public async toggleIsFavoriteById(
    offerId: string,
    action: number,
    userId: string,
  ): Promise<DocumentType<OfferEntity> | null> {
