...
export default class OfferGenerator implements OfferGeneratorInterface {
  ...
  public generate(): string {
    ...
    const rating = generateRandomValue(
      RatingRange.MIN,
      RatingRange.MAX,
      RatingRange.NUM_AFTER_DIGIT,
    );
    const typeOfHousing = getRandomItem<string>([
    ...
    const goods = getRandomItems<string>(
      Object.entries(OfferGood).map(([, value]) => value),
    ).join(';');
    ...
    const latitude = generateRandomValue(
      cityLatitude - LOCATION_DEVIATION,
      cityLatitude + LOCATION_DEVIATION,
      LOCATION_NUM_AFTER_DIGIT,
    );
    const longitude = generateRandomValue(
      cityLongitude - LOCATION_DEVIATION,
      cityLongitude + LOCATION_DEVIATION,
      LOCATION_NUM_AFTER_DIGIT,
    );
